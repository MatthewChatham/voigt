...
def read_data(fname, format):
    ...
    if format == "Q500/DMSM":
        ...
        # file encoding hardcoded for these files coming from the TGA on windows
...
def derivative(temp, mass):
    ...
    Return the derivative, and shifted temperature values to the mean of consecutive points
    """
    n = 1
    mass_diff = [mass[i+n] - mass[i] for i in range(len(mass)-n)]
    temp_diff = [temp[i+n] - temp[i] for i in range(len(temp)-n)]
    #return -1 * (np.diff(mass) / np.diff(temp)) , temp[:-1] + 0.5*np.diff(temp)
    return -1*np.asarray(mass_diff)/np.asarray(temp_diff), temp[:-n] + 0.5*np.asarray(temp_diff)

def med_smooth(temp, mass, window_size=30):
    """Use median value of window to smooth mass and temp values (temp values are unevenly spaced)"""
    ...
    for i in range(int(len(mass)/window_size)-1):
        #print(np.median(mass[i*window_size:(i+1)*window_size]))
        window = mass[i*window_size:(i+1)*window_size]
        window_within_bounds = np.where(np.abs(window) < 50)
        mass_smooth[i] = np.median(window[window_within_bounds])#mass[i*window_size:(i+1)*window_size])
        temp_smooth[i] = np.mean(temp[i*window_size:(i+1)*window_size])
    #temp_smooth = temp[::window_size]
...
def fft_filter(temp,dmdt):
    ...
    rft[-int(0.2*len(rft)):] = 0#[i/(ci+100) for ci,i in enumerate(rft[100:])]
...
def mean_smooth(temp, mass, window_size=2):
    ...
    mass_csum[window_size:] = mass_csum[window_size:] - mass_csum[:-window_size]
    mass_smooth = mass_csum[window_size - 1:] / window_size

    temp_csum = np.cumsum(temp)
    temp_csum[window_size:] = temp_csum[window_size:] - temp_csum[:-window_size]
    ...
    if negative_peaks == 'n':
        for model_num, basis_func in enumerate(spec['model']):                        
            ...
            if model_num >= n_peaks:
                ...
                center_min = x_min
                center_max = x_max
                # peaks can be tall, but set a limit on how small they can be to avoid overfitting
                amp_min = 0.3*y_max
                amp_max = np.inf
...
def update_spec_from_peaks(spec, free_peaks, max_peak_num, negative_peaks, **kwargs):
    ...
    # from the sample data, it seems like 20C, 50C, and 100C cover most of the major 

    peak_widths = (20./(x[1] - x[0]), 50./(x[1] - x[0]), 100./(x[1] - x[0]))
    
    p_peaks = np.asarray([])
    
    # initial signal to noise ratio for convolution wavelet transform peak finder
    snr = 1.5
    while len(p_peaks) == 0:
    ...
    #free peaks
    for cp in range(len(peak_indices), max_peak_num):#len(peak_indices) + free_peaks):
        spec['model'].append({'type': 'VoigtModel'})
...
def create_default_params_file():
    ...
    with open('params_file.txt', 'w') as p:
        ...
        p.write('#run start temp: temperature at which to consider the experiment "live" (C)\n')
...
def write_fig(fname, temp, mass, spec, output, negative_output, no_negative_peaks_found, 
    ...
    if multi_fit:
        ...
        fit_file_name = os.path.join(fit_dir, 'fit_' + str(model_num) + '_' + only_fname)
        if not os.path.isdir(fit_dir):
            os.mkdir(fit_dir)


    # matplotlib stuff
    f, (ax1, ax2, ax3, ax4) = plt.subplots(4, sharex=False, sharey=False, figsize=(10,20))
    ax1.plot(temp, mass)
    ...
    zoom_x_range_low = np.argmax(np.abs(best_fit)>0.01*max(output.best_fit))
    zoom_x_range_high = len(output.best_fit)-1 - np.argmax(np.abs(best_fit[::-1])>0.01*max(output.best_fit))
    ...
    try:         
        ax1.text(100, 145, 'Number of Negative Peaks: ' + str(len(neg_spec['model'])))
    except:
        ax1.text(100,145, 'Number of Negative Peaks: 0')
                 
    ax1.text(100, 140, 'chi2: ' + str(round(output.chisqr,2)))
    ax1.text(100, 135, 'BIC: ' + str(round(output.bic, 2))+ '   AIC: ' + str(round(output.aic, 2)))
    ...
    ax1.text(100,115,'Mass loss to amorphous carbon temp '+str(amorphous_carbon_temp) + 'C: ' + str(round(mass_at_rt - mass_at_amorphous_carbon_temp,2)) + ' mg --- ' + str(round((mass_at_rt - mass_at_amorphous_carbon_temp)/mass_at_rt*100, 2)) + '%')
    ax1.text(100,110,'Mass loss % (start mass - end mass)/(start mass)*100 between ' + str(round(mass_loss[0], 0)) +'C and ' +str(round(mass_loss[1], 0)) + 'C  : ' + str(round((mass_at_rt - mass_at_max)/mass_at_rt*100, 2)) + ' %')
    ax1.text(100,105,'Mass at ' + str(round(mass_loss[1], 0)) + ' C : ' + str(round(mass_at_max, 2)) + ' mg  --- ' + str(round(mass_at_max/mass_at_rt*100, 2)) + ' %')
    plt.xlabel('Temperature')


    if multi_fit == 0:
        plt.savefig(os.path.join(fit_dir,'fig_fit_'+only_fname[:-4]+'.png'), dpi=300)
    else:
        plt.savefig(os.path.join(fit_dir, 'fig_fit_'+str(model_num)+'_'+only_fname[:-4]+'.png'), dpi=300)
    ...
    with open(fit_file_name, 'w') as out:
        ...
        out.write('Mass at ' + str(round(rt, 0)) + ' C  : ' + str(mass_at_rt) + ' mg  --- 100 % \r\n')
        out.write('Mass at ' + str(round(mass_loss[1], 0)) + 'C : ' + str(mass_at_max) + ' mg  --- ' + str(mass_at_max/mass_at_rt*100) + ' % \r\n')
        out.write('Mass loss % (start mass - end mass)/(start mass)*100 between ' + str(round(mass_loss[0], 0)) +'C and ' +str(round(mass_loss[1], 0)) + 'C  : ' + str(round((mass_at_rt - mass_at_max)/mass_at_rt*100, 2)) + ' % \r\n')
        out.write('Mass loss to amorphous carbon temp'+str(amorphous_carbon_temp) + 'C: ' + str(round(mass_at_rt - mass_at_amorphous_carbon_temp,2)) + ' mg --- ' + str(round((mass_at_rt - mass_at_amorphous_carbon_temp)/mass_at_rt*100, 2)) + '%\r\n')
        out.write('Mass loss from '+str(run_start_temp) + ' C: ' + str(round(mass_at_run_start - mass_at_max,2)) + ' mg --- ' + str(round((mass_at_run_start - mass_at_max)/mass_at_run_start*100, 2)) + '%\r\n')
...
if __name__ == "__main__":
    ...
    if len(p_name) == 0:
        ...
        input_params = parse_params(p_name[0])
    ...
    try:
        ...
        print('invalid entry in params file: max peak num takes one integer number as input')
        exit(0)

    try:
        run_start_temp = float(input_params['run start temp'].strip())
    except:
        print('invalid entry in params file: run start temp takes one number as input')
        exit(0)

    try:
        mass_defect_warning = float(input_params['mass defect warning'].strip())
    except:
        print('invalid entry in params file: mass defect warning takes a number as input')
        exit(0)
    ...
    if input_params['negative peaks'].strip() == 'yes':
        ...
        if input_params['Temperature range to bound negative curve fitting'].strip() != 'None':
            if input_params['Temperature range to bound negative curve fitting'].strip() == 'full':
                ...
                try:
                    negative_peak_lower_bound, negative_peak_upper_bound = input_params['Temperature range to bound negative curve fitting'].strip().split(',')
                    neg_range = [float(negative_peak_lower_bound), float(negative_peak_upper_bound)]
                except:
                    print('invalid entry in params file: Temperature range to bound negative curve fitting takes two comma separated values, or full')
        else:
            neg_range = [-np.inf, np.inf]
        ...
        print('invalid entry in params file: negative peaks takes only yes/no')
        exit(0)

    if input_params['Temperature range to bound positive curve fitting'].strip() == 'full':
        fit_range = [-np.inf, np.inf]
    else:#if ',' not in input_params['Temperature range to bound positive curve fitting']:
        try:
            fit_range = [float(i.strip()) for i in input_params['Temperature range to bound positive curve fitting'].split(',')]
        except:
            print('invalid entry in params file: Temperature range to bound positive curve fitting takes two comma separated numbers, or full')
            exit(0)

    file_format = input_params['file format'].strip()
    if file_format not in ["Q500/DMSM", "TGA 5500", "Just Temp and Mass"]:
        print('invalid entry in params file: file format takes only "Q500/DMSM", "TGA 5500", or "Just Temp and Mass"')
        exit(0)

    #amorphous carbon temperature
    try:
        amorphous_carbon_temp = float(input_params['amorphous carbon temperature'].strip())
    except:
        print('invalid entry in params file: amorphous carbon temperature\n')
        exit(0)


    mass_loss = [0,0]
    try:
        mass_loss[0] = float(input_params['Temperature to calculate mass loss from'])
    except:
        print('invalid entry in params file: Temperature to calculate mass loss from\n')
        exit(0)

    try:
        mass_loss[1] = float(input_params['Temperature to calculate mass loss to'])
    except:
        print('invalid entry in params file: Temperature to calculate mass loss to')
        exit(0)
    ### 


    # number of fit options if the mass defect difference threshold for fit quality is exceeded
    ...
    for fname in flist:
        ...
        ### read in TGA data 
        data = read_data(fname, file_format)
        deriv = []
        ...
        around_max = np.where((temp > fit_range[1] - 1) & (temp < fit_range[1] + 1))
        mass_at_max = np.mean(mass[around_max])
        temp_at_max = np.mean(temp[around_max])
        temp = temp[in_range]
        mass = mass[in_range]

        around_run_start = np.where((temp > run_start_temp - 1) & (temp < run_start_temp + 1))
        mass_at_run_start = np.mean(mass[around_run_start])

        around_amorphous_carbon = np.where((temp > amorphous_carbon_temp - 1) & (temp < amorphous_carbon_temp))
        ...
        for free_peaks in range(0,max_peak_num - fixed_peaks):
            ...
            for model_attempt in range(n_attempts):
                ...
                # initial guess
                spec, peaks_found = update_spec_from_peaks(spec, free_peaks, max_peak_num, 'n')
                
                model, params = generate_model(spec, peaks_found, 'n', neg_range)
                                
                output = model.fit(spec['y'], params, x=spec['x'], method='least_sq', fit_kws={'ftol':1e-10})
        ...
        if negative_peaks_flag == 'y':
            ...
            # consider subtracting the fit from the peaks, and flipping that instead in future improvements
            ...
            #print(spec)
            neg_spec, peaks_found = update_spec_from_peaks(spec, 0, max_peak_num, 'y')
            #print(neg_spec)
            if neg_spec != 0:
                negative_model, negative_params = generate_model(spec, peaks_found, 'y', neg_range)
                
                #fitter = Minimizer('least_squares', params, **{'tol':1e-10})
                
                #fitter = fitter.minimize(method=method)
                
                negative_output = negative_model.fit(spec['y'], negative_params, x=spec['x'], method='least_sq', fit_kws={'ftol':1e-12})
                negative_components = negative_output.eval_components(x=spec['x'])
            else:
        ...
        if no_negative_peaks_found == 0:    
            for i, model in enumerate(neg_spec['model']):
                area += integrate.trapz(negative_components['m'+str(i)+'_'], spec['x'])
        ...
        if np.abs(area - (mass_at_rt - mass_at_max)/mass_at_rt*100) < mass_defect_warning:
            ...
            write_fig(fname, temp, mass, spec, output, negative_output, no_negative_peaks_found, 
                      components, negative_components, best_fit, model_param_array, multi_fit, 
                      mass_at_rt, rt, mass_at_run_start, mass_at_max, temp_at_max, amorphous_carbon_temp, 
                      mass_at_amorphous_carbon_temp, area, best_model_id)
        else:
            multi_fit = 1
            ...
            for model_num in range(len(model_array)):
                ...
                if no_negative_peaks_found == 0:    
                    for i, model in enumerate(neg_spec['model']):
                        area += integrate.trapz(negative_components['m'+str(i)+'_'], spec['x'])
                area_difference_array.append(np.abs(area - (mass_at_rt - mass_at_max)/mass_at_rt*100))
